...
async function writeReviews(reviews) {
  ...
  if (end < 0) throw new Error('Could not find the end of the REVIEWS array');
  ...
  const banner =
    '  /* ─────────────────────────────────────────────────────────────────\n' +
    '     GENERATED by tools/judgeme-sync.mjs on ' + new Date().toISOString() + '\n' +
    '     ' + reviews.length + ' published review' + (reviews.length === 1 ? '' : 's') +
      ' from Judge.me. Do not edit by hand — reviews\n' +
    '     are moderated in Judge.me and re-synced. Everything outside this\n' +
    '     array is editorial and is preserved by the sync.\n' +
    '     ───────────────────────────────────────────────────────────────── */\n';

  /* Drop a previous banner so they do not stack up on every run. */
  const before = src.slice(0, start).replace(
    /  \/\* ─+\n     GENERATED by tools\/judgeme-sync\.mjs[\s\S]*?─+ \*\/\n$/, '');

  await writeFile(path, before + banner + startMark + '\n' + body + src.slice(end), 'utf8');
  return path;
}
...
try {
  ...
  if (!process.env.JUDGEME_MOCK && (!cfg.token || !cfg.shopDomain)) {
    console.error(
      ...
      '  The token is private. Never commit it. See REVIEWS.md.\n');
    process.exit(1);
  }
  ...
  if (reviews.length && !withFit) {
    console.log(
      ...
      '     with the answers listed in REVIEWS.md, or change judgemeConfig.questions\n' +
