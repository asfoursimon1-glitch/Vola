...
async function writeCatalogue(products) {
  ...
  /* Windows git normalises this file to CRLF the moment anything commits
     ...
     post-commit checkout: the old endMark was a literal '\n  ];\n' and
     silently never matched '\r\n  ];\r\n'. Detecting the file's actual
     line ending and matching either style (via regex) survives both,
     and writing the generated banner/body back out in that same style
     keeps the file from ending up a mix of the two. */
  ...
  if (!endMatch) throw new Error('Could not find the end of the PRODUCTS array');
  ...
  const banner =
    '  /* ─────────────────────────────────────────────────────────────────' + eol +
    '     GENERATED by tools/shopify-sync.mjs on ' + new Date().toISOString() + eol +
    '     Do not edit by hand — re-run the sync instead. Everything outside' + eol +
    '     this array is editorial and is preserved by the sync.' + eol +
    '     ───────────────────────────────────────────────────────────────── */' + eol;

  const next = src.slice(0, start) + banner + startMark + eol + body + src.slice(end);
...
try {
  ...
  if (!process.env.SHOPIFY_MOCK && (!cfg.domain || !cfg.token)) {
    ...
    throw new Error(
      ...
      '  See docs/SHOPIFY.md for where to find them.');
  }
